...
function isFuel(card) {
    ...
    // Type explicitly mentions fuel
    if (type.includes('fuel')) return true;

    // Name is literally "Fuel" or starts with "Fuel ..."
    if (name === 'fuel' || name.startsWith('fuel ')) return true;

    // No type but ID like f001, f002, etc.
    if (!type && /^f\d{3,}$/i.test(id)) return true;

    return false;
}

// Map CSV row → card object
function mapRow(row) {
    const out = {
        name:           row['Card Name']        ?? '',
        type:           row['Card Type']        ?? '',
...
function frontImagePath(card) {
    ...
    if (!id) return ''; // no ID → no image (we log it elsewhere)
...
function applyFilters() {
    ...
    const tVal = lc(tEl.value);   // selected type filter
    const rVal = lc(rEl.value);   // selected rarity filter
    ...
    let rows = CARDS.filter(c => {
        ...
        if (tVal) {
            ...
            // Searching with no type filter → allow Fuel
            tOk = true;
        }
    ...
    // Sort (special case: rarity order)
    rows.sort((a, b) => {
        if (key === 'rarity') {
...
function loadCSV() {
    Papa.parse(CSV_URL, {
        ...
        skipEmptyLines: true,
        complete: (res) => {
            CARDS = res.data.map(mapRow);
            applyFilters();
        },
        error: (e) => {
            console.error(e);
            empty.textContent = 'Failed to load CSV.';
            empty.hidden = false;
        }
    });
}
...
function wireCards() {
    ...
    grid.querySelectorAll('.tile').forEach(tile => {
        ...
        tile.addEventListener('click', (e) => {
            if (e.target.closest('.mark-compare, .compare-btn')) return;
...
function renderCompare(a, b) {
    compareArea.innerHTML = [a, b].map(c => c ? compareBlock(c) : '').join('');
}
...
function compareBlock(c) {
  ...
  <div class="compare-card">
    ...
    <table class="meta-table">
      ...
      ${c.off_ability || c.off_text
        ? `<tr><td>Offensive</td><td>${[c.off_ability, c.off_text].filter(Boolean).join(' — ')}</td></tr>` : ''}
      ${c.def_ability || c.def_text
        ? `<tr><td>Defensive</td><td>${[c.def_ability, c.def_text].filter(Boolean).join(' — ')}</td></tr>` : ''}
